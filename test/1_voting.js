...
contract('Voting', function(accounts) {
  ...
  // Test #1 - This test evaluates if a proposal or initiative can be successfully added or saved so that it can be voted upon
  ...
  // Test #2 - This test evaluates if a "Yes" vote can be successfully cast on a proposal
  ...
  it("Successfully cast a 'Yes' vote on an initiative", function() {
   ...
   // Test #3 -  This test evaluates if a "No" vote can be successfully cast on a proposal
  ...
  // (5) This test evaluates if a vote can be counted if it is not a Y, N, or Abstain
  ...
  it("Successfully was unable to cast an invalid vote", function() {
    ...
    return web3.eth.getAccounts(function(error, accounts) {
      ...
      return Voting.deployed().then(function(instance) {
        ...
        return votingInstance.vote.call(0, 9, { from: account4 });  // Note here the vote cast is a "9" -- not a 1, 2, or 3
      }).then(function(data) {
  ...
  // (6) Throws an error if same account tries to vote twice on same initiative 
  ...
  // (7) Test to see that new initiative can be queued up to be voted on, even if initial initiative received only one vote
